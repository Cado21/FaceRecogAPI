...
const globalStruct = require('../struct/index');
const { DEFAULT_COMPENSATION, DEFAULT_DETAIL_PROFILE_DATA, USER_ROLE } = require('../utils/constants');
...
const createDetailProfileWithCompensation = async (session) => {
  const userCompensation = new Model.Compensation(DEFAULT_COMPENSATION);
  const savedCompensation = await userCompensation.save({ session });

  const detailProfile = new Model.DetailProfile({
    ...DEFAULT_DETAIL_PROFILE_DATA,
    compensationId: savedCompensation._id,
  });

  await detailProfile.save({ session });
  return detailProfile._id;
};

const employeeProfileById = async ({ userId, organizationId }) => {
  const employeeProfile = await Model.UserOrganization.findOne({ userId, organizationId })
    .populate({
      path: 'userId',
      populate: {
        path: 'roleId',
...
const employeeProfileList = async (organizationId, {
  page, limit, name,
}) => {
  const offset = page - 1;

  const role = await getDataRole(USER_ROLE.EMPLOYEE);
  const findQuery = { organizationId, roleId: role._id };
  const employeeProfiles = await Model.UserOrganization.find(findQuery)
    .populate({
      path: 'userId',
      populate: {
        path: 'roleId',
  ...
  const employeeList = employeeProfiles.filter((userOrg) => {
    const isAdmin = userOrg?.userId?.roleId?.name === USER_ROLE.ADMIN;
    const userFullName = userOrg?.userId?.fullname || '';
    const isFilteredName = userFullName && userFullName.toLowerCase().includes(name.toLowerCase());
    return (userOrg && !isAdmin && (userFullName === '' || isFilteredName));
  });

  return {
    list: employeeList,
    pagination: globalStruct.Pagination(
      page,
      limit,
      employeeList.length,
    ),
...
const updateUserProfileById = async ({ userId, organizationId }, payload, session) => {
  if (!userId || !organizationId) {
    throw new ResponseError(
      StatusCodes.BAD_REQUEST,
      'Please provide both userId and organizationId!',
      { userId, organizationId },
    );
  }
  const userExist = await getDataUser(userId, organizationId, session);
  if (!userExist) {
    throw new ResponseError(
      StatusCodes.BAD_REQUEST,
      `User with id: ${userId} are not exist in ${organizationId}`,
    );
  }
  const { detailProfileId } = userExist;
  if (detailProfileId) {
    const { compensation: compensationPayload, ...detailPayload } = payload;
    const updatedDetailProfile = await Model.DetailProfile.findOneAndUpdate(
      detailProfileId,
      detailPayload,
      { new: true, session },
    );
    await Model.Compensation.findOneAndUpdate(
      updatedDetailProfile.compensationId,
      compensationPayload,
      { session },
    );
  }
  const updatedProfile = Model.UserOrganization.findOne({ userId, organizationId })
    .populate({
      path: 'userId',
      populate: {
        path: 'roleId',
...
const findUserByEmail = async ({ email, role }, session) => {
  const roleEntity = await Model.Role.findOne({ name: role }).session(session);
  const user = await Model.User
    .findOne({ email, roleId: roleEntity._id })
    .session(session);

  return user;
};

const changePasswordByUserId = async (userId, newPassword, session) => {
...
module.exports = {
  ...
  createDetailProfileWithCompensation,
  employeeProfileById,
  findUserByEmail,
